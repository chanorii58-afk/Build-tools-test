...
local function getInfiniteBuildArgs(targetPos, root)
    -- Unfiltered input processing oversight
    -- Instead of spoofing a nearby block, we pass a raw table or NaN/target directly
    -- to exploit the lack of server-side input filtering on the targetBlock parameter.
    local unfilteredTarget = {
        Position = root and root.Position or targetPos,
        CFrame = CFrame.new(root and root.Position or targetPos)
    }
    
    -- Try passing the target position directly to see if the server processes it without filtering
    return nil, Enum.NormalId.Top, targetPos, targetPos
end
